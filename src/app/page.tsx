...
const Home = () => {
    ...
    const { user, spotifyToken, bgColor, playlists, sidebarCollapsed } = state;
            ...
            <div className={`flex rounded-xl p-5 pb-10`}>
                ...
                {currentPlaylist && (
                    <div className="flex column items-center relative ml-3">
                        <p className="absolute top-8 text-xs text-[var(--text-color)]">Playlist</p>
                        <p className="leading[2.7rem] text-5xl font-bold">{currentPlaylist.name}</p>
                        {currentPlaylist.description && (
                            <p className="absolute leading-3 text-xs text-[var(--text-color)] bottom-7 mb-1">
                                {currentPlaylist.description}
                            </p>
                        )}
        ...
        <div className="relative min-h-screen flex overflow-hidden">
            ...
            {user && (
                <main
                    ...
                    className={`h-[100%] rounded-lg flex-1 min-w-0 overflow-auto m-4 ${sidebarCollapsed ? "ml-[100px]" : "ml-[345px]"}`}
                >
