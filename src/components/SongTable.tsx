...
const SongTable: React.FC<SongTableProps> = ({ spotifyToken }): React.ReactElement => {
    ...
    const { playlistSongsFetched, setPlaylistSongsFetched, currentPlaylist, windowSize, isMobile } = useContext(
        AppContext,
    ) as AppContextType;
    ...
    const onDragEnd = (result: DropResult): void => {
        ...
        spotify
            .reorderTracksInPlaylist(playlistId, rangeStart, insertBefore)
            ...
            <table className={`w-full border-none ${isMobile ? 'mt-2' : ' mt-5'}`}>
                {!isMobile && (
                    <thead className="border-b border-[var(--border-color)]">
                        <tr className="flex">
                            {playlistColumnTitles.map(
                                (titleObj, index) => titleObj.render && renderTitle(titleObj.name, index),
                            )}
                        </tr>
                    </thead>
                )}
                    ...
                    <Droppable droppableId="droppable">
                        {(provided: DroppableProvided) => (
                            <tbody
                                ...
                                className="relative h-[100%] block overflow-y-scroll"
                            >
                                <SongItems songItems={songs} isTable={true} />
